...
class TimeBlock(nn.Module):
    ...
    def forward(self, X):
        ...
        # (BatchSize, Features, Height, Width)->(BatchSize, Features, NodeNumber, TimeAxis)
        X = X.permute(0, 3, 1, 2)
...
class STGCNBlock(nn.Module):
    ...
    def forward(self, X, A_hat):
        ...
        # A_hat -> (num_nodes, num_nodes), t.permute(1,0,2,3)-> (num_nodes, batch_size, num_timesteps, num_features)
...
class STGCN(nn.Module):
    ...
    def __init__(self, num_nodes, num_features, num_timesteps_input, num_timesteps_output, adj, device):
        ...
        self.block1 = STGCNBlock(
            in_channels=num_features, out_channels=64, spatial_channels=16, num_nodes=num_nodes)
